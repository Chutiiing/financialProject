...
# from alphagen_qlib.stock_data import MyStockData, FeatureType
from alphagen_qlib.my_stock_data import MyFeatureType, MyStockData
...
class Expression(metaclass=ABCMeta):
    @abstractmethod
    def evaluate(self, data: MyStockData, period: slice = slice(0, 1)) -> Tensor: ...
...
class Feature(Expression):
    def __init__(self, feature: MyFeatureType) -> None:
    ...
    def evaluate(self, data: MyStockData, period: slice = slice(0, 1)) -> Tensor:
        ...
        return data.data[start:stop, int(self._feature)-1, :]
...
class PairRollingOperator(Operator):
    ...
    def _unfold_one(self, expr: Expression,
                    data: MyStockData, period: slice = slice(0, 1)) -> Tensor:
        start = period.start - self._delta_time + 1
        stop = period.stop
