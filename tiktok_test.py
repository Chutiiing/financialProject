...
class factor_backtesting:
    ...
    def get_prev_period_group(self):
        ...
        # # 不考虑zaowupan的值，取两个因子的平均值
        # factor_df = factor_df.groupby(['Stock_Name', 'Trade_Date']).agg({
        #                         'Factor_Value': 'mean'})\
        #                      .reset_index()

        # # 改为变化率
        # factor_df['Factor_Value'] = factor_df.groupby('Stock_Name')['Factor_Value'].pct_change()
        # print("计算变化率")
        # print(factor_df)
        #
        # # 剔除开始的日期
        # factor_df = factor_df[factor_df['Trade_Date'] != '2022-08-09']

        # 获取股票代码
        stock_info_df = self.database_reader.read_from_db(
            "gogoalest3",
            f"""
                SELECT DISTINCT
                  CASE
        ...
        factor_df = pd.merge(factor_df, stock_info_df[['Stock_Name', 'Trade_Date', 'Stock_Code']], on=['Stock_Name', 'Trade_Date'], how='left') \
                      .drop(columns=['Stock_Name']) \
                      .dropna(subset=['Stock_Code'])        # 删除 'Stock_Code' 列中包含 NaN 的行
        ...
        def treat_na_then_rank_group(series):
            ...
            if series.notna().any():
                ...
                return pd.qcut(series.rank(method='first'),
                               q=self.group_counts,
                               labels=range(1, self.group_counts + 1)).astype(int)     # 标签是[1,self.group_counts + 1)
            # 传入的series全为nan
            else:
                # 在这里返回一个只包含 nan 的 Series，长度与传入的 series 相同
        ...
        # 删除含有 -1 的行（传入的全为nan）
        group_series = group_series[group_series != -1]
    ...
    def get_valid_stk_group_daily_rtn_df(self, ):
        if self.valid_stk_group_daily_rtn_df is None:
            ...
            # 强制转化为整数类型
            self.valid_stk_group_daily_rtn_df['Group'] = self.valid_stk_group_daily_rtn_df['Group'].astype(int)
    ...
    def plot(self, year_interested=None, month_interested=None):
        ...
        # 计算平均换手率
        group_turnover_df = group_daily_rtn_df[group_daily_rtn_df.Period_End_Date ==
                                                 group_daily_rtn_df.Trade_Date].copy(deep=True)
        group_turnover_df['Trade_Date'] = pd.to_datetime(group_turnover_df['Trade_Date'])

        def calculate_turnover_rate(group_df):
            # 过滤掉 'mcap' 为空的行
            group_df = group_df.dropna(subset=['mcap'])

            total_portfolio_value = group_df.groupby('Trade_Date')['mcap'].sum()
            prev_total_portfolio_value = total_portfolio_value.shift(fill_value=0)
            total_portfolio_value_diff = total_portfolio_value - prev_total_portfolio_value
            turnover_rate = (total_portfolio_value_diff.abs() / total_portfolio_value).mean()

            # 计算年化换手率
            trading_days_per_year = 252  # 假设一年有252个交易日
            annualized_turnover_rate = turnover_rate * trading_days_per_year

            return pd.Series([turnover_rate, annualized_turnover_rate],
                             index=['TurnoverRate', 'AnnualizedTurnoverRate'])

        group_turnover_df = group_turnover_df.groupby('Group').apply(calculate_turnover_rate).reset_index()
        print("group_turnover_df.columns??")
        print(group_turnover_df.columns)

        # Compute the IC values
        # spearmanr()用于计算秩相关系数
        # 清理包含 NaN 或 inf 的行
        cleaned_df = group_daily_rtn_df.dropna(subset=['Daily_Return', 'Prev_Factor_Value'])
        cleaned_df = cleaned_df.replace([np.inf, -np.inf], np.nan).dropna()
        # 计算秩相关系数
        rank_ic = cleaned_df.groupby('Trade_Date').apply(
            lambda df: stats.spearmanr(df['Daily_Return'], df['Prev_Factor_Value'])[0])
        normal_ic = cleaned_df.groupby('Trade_Date').apply(
            lambda df: stats.pearsonr(df['Daily_Return'], df['Prev_Factor_Value'])[0])
        ...
        fig, axes = plt.subplots(figsize=(32, 32), nrows=3, ncols=2)

        # 添加总标题
        plt.suptitle(f'{self.freq}_{self.stock_universe}因子', fontsize=28)
        ...
        # plot 5:平均换手率
        sns.barplot(data=group_turnover_df, x='Group',
                    y='TurnoverRate', ax=axes[2, 0])
        axes[2, 0].set_xlabel('组', labelpad=15)
        axes[2, 0].set_ylabel('周期平均换手率', labelpad=15)
        axes[2, 0].set_title(title_prefix + '根据因子值分组后的各组股票周期的平均换手率')

        plt.savefig(self.hostname + f'{self.freq}_{self.stock_universe}因子.png')

        # plot 6:平均换手率
        sns.barplot(data=group_turnover_df, x='Group',
                    y='AnnualizedTurnoverRate', ax=axes[2, 1])
        axes[2, 1].set_xlabel('组', labelpad=15)
        axes[2, 1].set_ylabel('周期年化换手率', labelpad=15)
        axes[2, 1].set_title(title_prefix + '根据因子值分组后的各组股票周期的年化换手率')
...
if __name__ == '__main__':
    ...
    backtest2 = factor_backtesting(start_date='2022-08-09',
                                   end_date='2023-08-09',
                                   factor_table=None,
                                   group_counts=10,
                                   freq='D',
    ...
    backtest3 = factor_backtesting(start_date='2022-08-09',
                                   end_date='2023-08-09',
                                   factor_table=None,
                                   group_counts=10,
                                   freq='W',
    ...
    backtest4 = factor_backtesting(start_date='2022-08-09',
                                   end_date='2023-08-09',
                                   factor_table=None,
                                   group_counts=10,
                                   freq='M',
